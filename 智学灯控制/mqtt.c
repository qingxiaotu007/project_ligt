...
uint32_t mqtt_cjson_parse(char *pbuf)
{
	...
	else
	{
		...
		if((Params = cJSON_GetObjectItem(Json,"params")) != NULL)
		{
			...
			if((Item = cJSON_GetObjectItem(Params,"light1")) != NULL)//匹配子对象2中的成员 "NO"
			{
				led1 = Item->valueint;
				light1 = 1;//切换手动模式
				TIM_SetCompare2(TIM4,led1);
			}
			if((Item = cJSON_GetObjectItem(Params,"light2")) != NULL)//匹配子对象2中的成员 "led1"
			{
				led2 = Item->valueint;
				light2 = 1;//切换手动模式
				TIM_SetCompare3(TIM4,led2);
			}
			if((Item = cJSON_GetObjectItem(Params,"light3")) != NULL)//匹配子对象2中的成员 "led2"
			{
				led3 = Item->valueint;
				light3 = 1;//切换手动模式
