...
volatile extern uint8_t  window =0; //窗户参数
...
{
    uint8_t NO_state	= GPIO_ReadOutputDataBit(GPIOA,GPIO_Pin_5);
	
	uint8_t HS_state	=GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_9);
	uint8_t mq_state 	= Mq_read() ? 0:1;
	uint8_t water_state = Water_read();
	uint8_t mq_size		= 100-(Get_Adc2_Average(ADC_Channel_8,5)*100/4096);
	uint8_t water_size 	= 100-(Get_Adc_Average(ADC_Channel_1,10)*100/4096);
	uint8_t temp 		=0;
	uint8_t humi 		=0;
	...
	sprintf(mqtt_post_msg,
		"{\"method\":\"thing.service.property.set\",\"id\":\"1596870845\",\"params\":{\
		\"mq2_size\":%d,\
		\"water_size\":%d,\
		\"water\":%d,\
		\"mq2\":%d,\
...
uint32_t mqtt_cjson_parse(char *pbuf)
{
	...
	else
	{
		...
		if((Params = cJSON_GetObjectItem(Json,"params")) != NULL)
		{
			...
			if((Item = cJSON_GetObjectItem(Params,"window")) != NULL)//匹配子对象2中的成员 "window"
			{
				printf("%s: %d \r\n",Item->string,Item->valueint);
				window = (Item->valueint)/4;
				//控制继电器
